...
class ManagerDialog(wx.Dialog):
    def __init__(
        ...
        self.speakRoleCb = wx.CheckBox(
            self, -1, _("Speak role such as button, edit box , link etc. ")
        )
        ...
        self.volumeSlider = VoiceSettingsSlider(
            self, wx.ID_ANY, minValue=0, maxValue=100, name=_("Add-on Volume")
        )
        ...
        buttonsSizer.AddMany(
            [(self.useTheme), (self.removeTheme), (self.addTheme), (cancelButton)]
        )
    ...
    def onSpeakRoleCbCheck(self, evt):
        ...
        if not cbVal:
            speech.getSpeechTextForProperties = (
                audioThemeHandler.hook_getSpeechTextForProperties
            )
        else:
            speech.getSpeechTextForProperties = (
                audioThemeHandler.NVDA_getSpeechTextForProperties
            )
        ...
        if selectedTheme.isActive:
            if (
                gui.messageBox(
                    ...
                    _(
                        "The {themeName} is the currently active theme. Do you realy want to remove it?"
                    ).format(themeName=selectedTheme.name),
                    # Translators: the title of the message box that ask the user if he/she realy want to remove the active theme.
                    _("Removing The Active Theme"),
            ...
            if (
                gui.messageBox(
                    ...
                    _(
                        "Do you want to remove the {themeName} theme permanently?"
                    ).format(themeName=selectedTheme.name),
    ...
    def onAddClick(self, event):
        filename = helpers.showFileDialog(
            self,
            ...
            ext="atp",
            extHint=_("Audio Theme Packages"),
        )
        if not filename:
            return
    ...
    def refresh(self):
        ...
        self.themesList.SetItemState(
            self.activeIndex, wx.LIST_STATE_FOCUSED, wx.LIST_STATE_FOCUSED
        )
